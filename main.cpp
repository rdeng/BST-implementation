...
int main(int argc, char* argv[])
{
	...
	//create a BST 
	BST<std::string> test;
        //keep looping untile the end of file
	while(!in.eof())
        {
            //get a line and store it in name
            getline(in,name);
            if(name != "")
            {
                //insert name into test tree
                test.insert(name);
            }
        }
        ...
        //print out size and height of the tree
        cout << "Size of tree: " << size << "\n";
        cout << "Height of tree: " << height << "\n";

        //create two BST Iterator 
        BST<std::string>::iterator findI = test.begin();
        BST<std::string>::iterator nullIte = test.end();

        //keep looping until user decide to quit
        while(1)
        {
            ...
            //break and exit if user decide to not search again
            if(name == "n")
            {
                break; 
            }
